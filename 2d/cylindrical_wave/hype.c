...
int main(int argc,char **argv) {
    ...
    Ctx.u_bnd_grad   = allocate5d(ym+2, xm+2, 4, N_gp2, DIM); // 4->number of faces, 2->number of quadraure points
    Ctx.F            = allocate2d(ym, xm+1);
    Ctx.G            = allocate2d(ym+1, xm);
    Ctx.gradphiFace_x = allocate3d(4, N_gp2, nDOF);
    Ctx.gradphiFace_y = allocate3d(4, N_gp2, nDOF);
    
    // Find the value of basis functions and gradients  
