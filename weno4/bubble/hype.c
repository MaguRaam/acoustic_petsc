...
int main(int argc,char **argv) {
    ...
    for (q = 0; q < N_gp2; ++q) {
        for (k = 0; k < nDOF; ++k) {
            set_element_3d(Ctx.phiFace, 0, q, k, basis(-0.5, x_gp2[q], k)); // Left face
            set_element_3d(Ctx.phiFace, 1, q, k, basis( 0.5, x_gp2[q], k)); // Right face 
            set_element_3d(Ctx.phiFace, 2, q, k, basis(x_gp2[q], -0.5, k)); // Bottom face 
            set_element_3d(Ctx.phiFace, 3, q, k, basis(x_gp2[q],  0.5, k)); // Top face 
        }
    }
